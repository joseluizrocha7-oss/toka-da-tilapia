...
name: salvar
description: >
  Salva o trabalho do TheRockzOS no GitHub (commit + push). Na primeira vez configura o repositório
...
Skill de uma função só: garantir que o trabalho da agência está no GitHub.
...
1. Perguntar:
   ...
   > 2. Não, vou criar agora — me dá um nome pro repositório (ex: rockz-os)"

2. **Se opção 1:** rodar `git init`, `git add .`, `git commit -m "Setup inicial do TheRockzOS"`, `git branch -M main`, `git remote add origin <URL>`, `git push -u origin main`.
...
- Se o push falhar por divergência, avisar e oferecer `git pull --rebase` antes de tentar de novo
- Se o usuário ainda não tiver `git` configurado, perguntar e configurar com `git config --global` na primeira vez
